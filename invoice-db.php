...
WHERE trx_id = '".$_GET['trx_id']."'"); //user_id = '$cm_user_id'"; //
...
//items
$query = mysqli_query($con,"select  * from products,orders where  orders.trx_id = '".$_GET['trx_id']."' "); //products.product_id = '".$orders['product_id']."'");
$sql = mysqli_query($con,"select  DISTINCT product_title from products,orders where products.product_id = '".$orders['product_id']."'");
...
while($products = mysqli_fetch_array($query)){
	...
	$pdf->Cell(105	,5,$item['product_title'],1,0);
	//add thousand separator using number_format function
	$pdf->Cell(25	,5,$orders['qty'],1,0); //$orders['qty']),1,0);
