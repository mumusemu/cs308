...
if(isset($_POST["email"]) && isset($_POST["password"])){
	...
	$sql_user = "SELECT * FROM user_info WHERE email = '$email' AND password = '$password' AND  Lv = 'U'";
    $sql_manager = "SELECT * FROM user_info WHERE email = '$email' AND password = '$password' AND  Lv = 'P'";
	$sql_smanager = "SELECT * FROM user_info WHERE email = '$email' AND password = '$password' AND  Lv = 'S'";

    $run_query_manager = mysqli_query($con,$sql_manager);
    $run_query_user = mysqli_query($con,$sql_user);
    $run_query_smanager = mysqli_query($con,$sql_smanager);

	$count_smanager = mysqli_num_rows($run_query_smanager);
    $count_manager = mysqli_num_rows($run_query_manager);
	$count = mysqli_num_rows($run_query_user);
	//if user record is available in database then $count will be equal to 1
	if($count == 1) {
        $row = mysqli_fetch_array($run_query_user);
	...
	else if($count_smanager == 1){
        $row = mysqli_fetch_array($run_query_user);
        $_SESSION["uid"] = $row["user_id"];
        $_SESSION["name"] = $row["first_name"];
        $ip_add = getenv("REMOTE_ADDR");

	    echo "salesmanager_success";
	    exit();
    }
    else{
