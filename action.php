...
if(isset($_POST["category"])){
	...
	echo "
		<div class='nav nav-pills nav-stacked'>
			<li class='active'><a href='#'style='background-color:thistle'><h4><strong>Categories</strong></h4></a></li>
	";
	...
	if(mysqli_num_rows($run_query) > 0){
		while($row = mysqli_fetch_array($run_query)){
			...
			echo "
					<li><a href='#' style='color:purple' class='category' cid='$cid'>$cat_name</a></li>
			";
		}
		echo "</div>";
	}
...
if(isset($_POST["brand"])){
	...
	echo "
		<div class='nav nav-pills nav-stacked'>
			<li class='active'><a href='#'style='background-color:thistle'><h4><strong>Brands</strong></h4></a></li>
	";
	if(mysqli_num_rows($run_query) > 0){
		while($row = mysqli_fetch_array($run_query)){
			$bid = $row["brand_id"];
			...
			echo "
					<li><a href='#' style='color:purple' class='selectBrand' bid='$bid'>$brand_name</a></li>
			";
		}
		echo "</div>";
	}
...
if(isset($_POST["getProduct"])){
	...
	$product_query = "SELECT * FROM products p WHERE p.product_id > 0 LIMIT $start,$limit";
			...
			echo "
				<div class='col-md-4'>
							<div class='panel panel-default'>
								<div class='panel-heading'>$pro_title<h6 style='float: right;'><b>In stock: $pro_stock</b></h6>
								</div>
								<div class='panel-body'>
									<a href ='./product.php?pid=$pro_id'>
									<img src='product_images/$pro_image' style='width:270px; height:240px;'/>
									</a>
								</div>
...
if(isset($_POST["get_seleted_Category"]) || isset($_POST["selectBrand"]) || isset($_POST["search"])){
	if(isset($_POST["get_seleted_Category"])){
		...
		$sql = "SELECT * FROM products p WHERE p.product_cat = '$id' ORDER BY product_id";
	}else if(isset($_POST["selectBrand"])){
		$id = $_POST["brand_id"];
		$sql = "SELECT * FROM products p WHERE p.product_brand = '$id'";
	}else {
		$keyword = $_POST["keyword"];
		$sql = "SELECT * FROM products p WHERE p.product_keywords LIKE '%$keyword%'";
	}
				...
				if (!isset($_SESSION["uid"])) {
					...
					echo '
						...
						<form action="https://www.sandbox.paypal.com/cgi-bin/webscr" method="post">
							...
							<input type="hidden" name="business" value="sbozdogan@sabanciuniv.edu">
							...
							while($row=mysqli_fetch_array($query)){
								$x++;
								echo  	
									$pp_checkout_btn .='<input type="hidden" name="item_name_'.$x.'" value="'.$row["product_title"].'">
							...
							echo   
								$pp_checkout_btn .= '<input type="hidden" name="return" value="http://localhost/team16/payment_success.php"/>
					                <input type="hidden" name="notify_url" value="http://localhost/team16/payment_success.php">
									<input type="hidden" name="cancel_return" value="http://localhost/team16/cancel.php"/>
									<input type="hidden" name="rm" value="2">
									<input type="hidden" name="currency_code" value="USD"/>
									<input type="hidden" name="custom" value="'.$_SESSION["uid"].'"/>
									<input style="float:right;margin-right:80px;" type="image" name="submit"
										src="https://cdn-images-1.medium.com/max/1000/1*MqdZnOy5ySk8PIUbUtt5Cg.png" alt="PayPal Checkout"
